...
int main() {
    ...
    const char* outputFile = "output_grayscale3.bmp";
    ...
    // 정보 헤더를 새로 생성 (호환성 문제 해결)
    BMPInfoHeader newInfoHeader = { 0 };
    newInfoHeader.size = sizeof(BMPInfoHeader);
    newInfoHeader.width = infoHeader.width;
    newInfoHeader.height = infoHeader.height;
    newInfoHeader.planes = 1;
    newInfoHeader.bitCount = 8;
    newInfoHeader.compression = 0;
    newInfoHeader.sizeImage = newRowSize * IMAGE_HEIGHT;
    newInfoHeader.xPelsPerMeter = infoHeader.xPelsPerMeter;
    newInfoHeader.yPelsPerMeter = infoHeader.yPelsPerMeter;
    newInfoHeader.clrUsed = 256;
    newInfoHeader.clrImportant = 256;

    // 파일 헤더 생성
    BMPFileHeader newFileHeader = fileHeader; // 원본의 type, reserved 필드는 유지
    newFileHeader.size = sizeof(BMPFileHeader) + sizeof(BMPInfoHeader) + paletteSize + newInfoHeader.sizeImage;
    ...
    // 그레이스케일 팔레트 생성 및 쓰기
    for (int i = 0; i < 256; i++) {
        uint8_t color[4] = { (uint8_t)i, (uint8_t)i, (uint8_t)i, 0 }; // Blue, Green, Red, Reserved
    ...
    for (int y = IMAGE_HEIGHT - 1; y >= 0; y--) {
        ...
        // 패딩 쓰기
        if (newPadding > 0) {
            fwrite(paddingBytes, 1, newPadding, outFile);
        }
    }
